...
modifierList
  : ( visibilityKeyword | VirtualKeyword | stateMutability | modifierInvocation | overrideSpecifier )* ;
...
expression
  : expression ('++' | '--') # SuffixOperation
  | 'new' typeName # NewExpression
  | expression '[' expression? ']' # IndexAccess
  | expression '[' expression? ':' expression? ']' # IndexRangeAccess
  | expression '.' identifier # MemberAccess
  | PayableKeyword '(' expression ')' # PayableExpression
  | elementaryTypeName '(' expression ')' # TypeConversion
  | expression '{' nameValueList '}' # ValueExpression
  | expression functionCallOptions? '(' functionCallArguments ')' # FunctionCallWithOptions
  | '(' expression ')' # ParenExpression
  | ('++' | '--') expression # PrefixOperation
  | ('+' | '-') expression # UnaryPrefixOperation
  | ('after' | 'delete') expression # KeywordPrefixOperation
  | '!' expression # NotOperation
  | '~' expression # BitNotOperation
  | expression '**' expression # ModOperation
  | expression ('*' | '/' | '%') expression # MulOperation
  | expression ('+' | '-') expression # AddOperation
...
// keywords other than RESERVED can also be a name.
identifier
  : (allKeywords | 'from' | 'calldata' | 'receive' | 'callback' | 'revert' | 'error' | Identifier) ;
...
visibilityKeyword : PublicKeyword | PrivateKeyword | ExternalKeyword | InternalKeyword ;
...
allKeywords: AnonymousKeyword | BreakKeyword | ConstantKeyword |
ImmutableKeyword | ContinueKeyword | LeaveKeyword |
IndexedKeyword | visibilityKeyword | PayableKeyword |
